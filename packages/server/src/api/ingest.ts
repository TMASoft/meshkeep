 ...
 * depend on which mode drove the radio. Everything is idempotent: messages
 * dedupe on their content hash, contacts/self are plain upserts.
 */
  ...
  for (const item of items) {
    ...
    if (message) {
      ...
      if (item.status === "delivered" || item.status === "failed") {
        const updated = store.updateMessageStatusByContent({ ...normalized, status: item.status });
        if (updated) {
