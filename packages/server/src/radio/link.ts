...
import { CONTACT_TYPE_FROM_ADV } from "@meshkeep/shared";
...
export interface RadioLinkDeps {
  /** profile id this link runs, or null for the implicit env/override "default" link. */
  key: number | null;
  /** Human-readable identity for log lines when several links run concurrently. */
  label: string;
  db: Db;
  store: Store;
  bus: Bus;
  config: ServerConfig;
  channelReadTimeoutMs?: number;
  connectionFactory?: typeof createConnection;
  /** Never inspects sibling links — the hub owns topology, a link only owns its own connection. */
  getEffectiveSettings: () => ConnectionSettings;
  /** Returns a conflict reason (e.g. BLE exclusivity) to block a connect attempt, or null to proceed. */
  beforeConnect?: () => string | null;
  /** Called after every state transition — the hub re-publishes its own aggregated status.changed. */
  onStateChange?: () => void;
...
/**
 * Owns one physical radio connection end-to-end: transport lifecycle,
 * reconnect/backoff, the outbound retry queue, and every radio-op write.
 * Everything the radio tells us is persisted through the shared `Store` and
 * re-broadcast on the shared `Bus`, tagged with this link's resolved radio id.
 */
export class RadioLink {
  ...
  private readonly channelReadTimeoutMs: number;
  private readonly connectionFactory: typeof createConnection;
  private readonly getEffectiveSettings: () => ConnectionSettings;
  private readonly beforeConnect?: () => string | null;
  ...
  // self public key on connect; seeded so stored data is visible before the
  // first reconnect.
  private radioId: number | null;
  private standby: boolean;
  private stopped = false;
  private draining = false;
  private drainAgain = false;
  // MeshCore responses have no request ID, so only one hand-written command
  // wrapper may listen for its generic response events at a time — per link.
  ...
  // flight (per link) until its acknowledgement arrives or times out.
  private directSendQueue: Promise<void> = Promise.resolve();
  private pendingDirectDelivery: PendingDirectDelivery | null = null;
  // Outbound retry worker: one drain loop at a time per link; a kick during a
  // run requeues one more pass, and a timer fires when a backed-off entry is due.
  private outboundRunning = false;
  private outboundKickAgain = false;
  private outboundTimer: NodeJS.Timeout | null = null;

  constructor(deps: RadioLinkDeps) {
    this.key = deps.key;
    ...
    this.channelReadTimeoutMs = deps.channelReadTimeoutMs ?? 5_000;
    this.connectionFactory = deps.connectionFactory ?? createConnection;
    this.getEffectiveSettings = deps.getEffectiveSettings;
    this.beforeConnect = deps.beforeConnect;
    this.onStateChange = deps.onStateChange;
    this.radioId = deps.initialRadioId;
    this.standby = deps.initialStandby;
  }
  ...
  /** The radio this link is connected to (or last connected to), or null before any sync. */
  getRadioId(): number | null {
    return this.radioId;
  }
  ...
  transport(): ConnectionSettings["connection"] {
    return this.getEffectiveSettings().connection;
  }

  /** The active radio id, or throw — for write/radio-op paths that require a synced identity. */
  private requireRadio(): number {
  ...
  async start(): Promise<void> {
    if (this.getEffectiveSettings().connection === "none") {
  ...
  async release(): Promise<void> {
    this.standby = true;
    this.store.setLinkStandby(this.key, true);
    await this.teardown();
    this.setState("standby", null);
  }
  ...
  async claim(): Promise<void> {
    this.standby = false;
    this.store.setLinkStandby(this.key, false);
  ...
  /** Tear down and reconnect (or settle in the right idle state) after this link's settings changed. */
  ...
  private async connect(): Promise<void> {
    if (this.stopped || this.standby || this.connection) return;
  ...
  private scheduleReconnect(): void {
    if (this.stopped || this.standby || this.reconnectTimer) return;
  ...
  private async initialSync(connection: Connection, generation: number, signal: AbortSignal): Promise<void> {
    ...
    // else: every subsequent write in this sync is scoped to it. Persist it so
    // a restart shows this radio's data before this link reconnects.
    const radioId = this.store.resolveRadio(self.publicKey, self.name);
    this.radioId = radioId;
    this.store.setLinkLastRadio(this.key, radioId);
    ...
    try {
      ...
      if (drift > CLOCK_DRIFT_TOLERANCE_SECS) {
        console.log(`[radio:${this.label}] device clock drifted ${drift}s; syncing`);
        await this.awaitCurrent(connection.syncDeviceTime(), connection, generation, signal);
      }
    } catch (error) {
      if (error instanceof LifecycleCancelledError) throw error;
      console.warn(`[radio:${this.label}] could not check device time`, error);
    }
  ...
  private async drainConnection(
    ...
    try {
      ...
      console.error(`[radio:${this.label}] failed draining messages`, error);
    } finally {
      this.draining = false;
  ...
  /**
   * Nudge this link's outbound worker to re-evaluate due entries now (e.g.
   * after a send/retry touched its queue). Safe no-op when not connected —
   * queuing/retry/cancel are store-level operations that work whether or not
   * this link currently exists, so they live on the hub, not here; the hub
   * calls this afterward to wake up delivery when a link is available.
   */
  kickOutbound(): void {
    void this.processOutboundQueue();
  }
  ...
  /**
   * Drain due outbound entries one at a time while this link's radio is
   * available. Success removes the entry (DMs then follow the normal
   * ack/timeout path); a hand-off failure backs the entry off, and a radio
   * that drops mid-send leaves the entry pending without burning an attempt.
   */
  private async processOutboundQueue(): Promise<void> {
    ...
    try {
      while (!this.stopped && !this.standby && (this.state === "connected" || this.state === "syncing")) {
        const connection = this.connection;
        if (!connection || this.radioId === null) break;
        const due = this.store.takeDueOutbound(this.radioId, nowSecs());
        if (!due.length) break;
        const entry = due[0]!;
        try {
          if (entry.kind === "dm") {
  ...
  private scheduleOutboundTimer(): void {
    ...
    if (this.stopped || this.standby) return;
    ...
    const next = this.store.nextOutboundAttemptAt(this.radioId);
    if (next === null) return;
  ...
  async setChannel(idx: number, name: string, secretHex: string): Promise<Channel> {
    ...
    this.store.upsertChannel(this.requireRadio(), channel);
    return channel;
  }
  ...
  async deleteChannel(idx: number): Promise<void> {
    ...
    this.store.deleteChannel(this.requireRadio(), idx);
  }
  ...
  private async pollBattery(
    ...
    if (!connection || this.radioId === null) return;
