...
/**
 * Operator-triggered connectivity probe. Deliberately absent from
 * EXTERNAL_EVENT_TYPES: it is never matched by subscription filters and is only
 * produced by `POST /webhooks/:id/test`, so a receiver can distinguish it from
 * mesh traffic by `type` alone and safely ignore it.
 */
export const WEBHOOK_TEST_EVENT_TYPE = "webhook.test";
export const externalWebhookTestEventSchema = z.object({
  ...externalEnvelopeFields,
  type: z.literal(WEBHOOK_TEST_EVENT_TYPE),
  data: z.object({
    test: z.literal(true),
...
export type ExternalWebhookTestEvent = z.infer<typeof externalWebhookTestEventSchema>;

/** Builds the signed test envelope; carries no mesh data, sensitive or otherwise. */
export function buildWebhookTestEvent(input: {
  id: string;
  subscriptionId: number;
  label: string;
  occurredAt?: string;
}): ExternalWebhookTestEvent {
  return externalWebhookTestEventSchema.parse({
    id: input.id,
    type: WEBHOOK_TEST_EVENT_TYPE,
    eventVersion: 1,
    occurredAt: input.occurredAt ?? new Date().toISOString(),
    source: { product: "meshkeep", apiVersion: "v1", radioId: 0 },
    data: { test: true, subscriptionId: input.subscriptionId, label: input.label },
  });
}
