...
- **Network map** — global map.meshcore.io mirror (server-side cached, rate-friendly) with your local mesh overlaid; use self-hosted tiles or an offline local-only canvas when privacy or connectivity requires it
...
a BLE radio near your _browsing_ device is often better served by browser-direct WebBLE.
...
| `MESHKEEP_MAP_TILES_ENABLED`              | `true`                                           | set `false` for an offline basemap-free canvas; local contacts and cached node data remain available |
...
### Map privacy and offline use

The global node index is fetched by the MeshKeep server and cached on disk; browsers request
only the cached result from MeshKeep. Basemap tiles are different: when enabled, the browser
requests each visible tile directly from `MESHKEEP_MAP_TILES_URL`. That tile provider can observe
the browser IP address and the map areas and zoom levels viewed. Point the URL at a self-hosted
tile service (a root-relative URL uses the same MeshKeep origin), or set
`MESHKEEP_MAP_TILES_ENABLED=false` to make no tile requests. The Network view identifies the
active tile mode and offers Refresh if the node index or tiles fail to load.
